...
export function CopyButtons({ url, markdown, title }: CopyButtonsProps) {
  ...
  const [chatOption, setChatOption] = useState<string>("discuss");
  ...
  const handleChatWithChatGPT = () => {
    ...
    const prompt = `${promptPrefix}Article: "${title}" by Chase Adams, a software engineer.\n\n${markdown}`;
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="relative" ref={dropdownRef}>
        <div className="flex gap-0">
          <button
            onClick={handleChatWithChatGPT}
            className="flex flex-1 items-center justify-center gap-2 rounded-l-lg border border-r-0 border-foreground/20 bg-background px-4 py-2 text-sm font-medium uppercase tracking-[0.1em] text-foreground/80 transition-colors hover:bg-foreground/5 hover:text-foreground"
        ...
        {isDropdownOpen && (
          <div className="absolute left-0 right-0 top-full z-10 mt-1 rounded-lg border border-foreground/20 bg-background shadow-lg">
            {CHAT_OPTIONS.map((option) => {
              ...
              return (
                <a
                  ...
                  onClick={() => {
                    setChatOption(option.value);
                    setIsDropdownOpen(false);
                  }}
                  className={`block w-full px-4 py-2 text-left text-sm !text-foreground/80 !no-underline transition-colors first:rounded-t-lg last:rounded-b-lg hover:bg-foreground/5 hover:!text-foreground ${
                    chatOption === option.value ? "bg-foreground/10 font-medium" : ""
                  }`}
                >
                  {option.label}
                </a>
