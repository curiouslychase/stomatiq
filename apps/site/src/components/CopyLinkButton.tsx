        ...
        {isDropdownOpen && (
          <div className="absolute left-0 right-0 top-full z-10 mt-1 rounded-lg border border-foreground/20 bg-background shadow-lg">
            {CHAT_OPTIONS.map((option) => (
              <div
                key={option.value}
                className="flex items-center gap-2 first:rounded-t-lg last:rounded-b-lg hover:bg-foreground/5"
              >
                <a
                  ...
                  className="flex-1 px-4 py-2 text-left text-sm !text-foreground/80 !no-underline transition-colors"
                >
                  {option.label}
                </a>
                <button
                  onClick={async () => {
                    const url = getChatUrl(option.value);
                    await navigator.clipboard.writeText(url);
                  }}
